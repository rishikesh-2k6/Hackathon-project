...
# ⚠️ YOUR API KEY ⚠️
# ---------------------------------------------------------
MY_API_KEY = "AIzaSyCkix6kWxHKStnCRVuBCAOnAq8rogFJHY8"
...
def get_ai_commands(user_text, current_tab="Unknown"):
    ...
    prompt = f"""
    You are a PowerPoint Instructor. 
    User may provide multiple tasks (e.g., 'add a slide and change background').
...
class Automator:
    ...
    def auto_click(self, control):
        try: control.GetInvokePattern().Invoke(); return True
        except: pass
        try: control.GetTogglePattern().Toggle(); return True
        except: pass
        try: control.GetSelectionItemPattern().Select(); return True
        except: pass
        try:
            r = control.BoundingRectangle
            x, y = (r.left + r.right) // 2, (r.top + r.bottom) // 2
            auto.Click(x, y); return True
        except: return False

    def warm_up_control(self, control):
        try:
            r = control.BoundingRectangle
            auto.MoveTo((r.left + r.right) // 2, (r.top + r.bottom) // 2)
            time.sleep(0.05)
        except: pass
    ...
    def execute_single_step(self, cmd, ghost, ppt, auto_mode=False):
        ...
        if tab.Exists(0, 1):
            ...
            txt = f"Auto opening '{cmd.tab_name}'" if auto_mode else f"Click '{cmd.tab_name}'"
            ghost.point_at((r.left+r.right)//2, (r.top+r.bottom)//2, txt)
            
            if auto_mode: self.auto_click(tab)
            else: 
        ...
        if btn.Exists(0, 1):
            ...
            txt = f"Auto clicking '{cmd.button_name}'" if auto_mode else f"Click '{cmd.button_name}'"
            ghost.point_at((r.left+r.right)//2, (r.top+r.bottom)//2, txt)
            
            if auto_mode: self.auto_click(btn)
            else:
...
class WorkerApp(ctk.CTk):
    ...
    def toggle_auto_mode(self):
        ...
        if self.auto_mode:
            self.btn_auto.configure(text="🤖 Auto ON", fg_color="#2f3b63", text_color="#e5e7eb")
            self.update_status_safe("Auto mode enabled")
        else:
            self.btn_auto.configure(text="🤖 Auto", fg_color="#1e2233", text_color="#a9b1d6")
            self.update_status_safe("Manual mode enabled")
    ...
    def redo_action(self): 
        if self.last_input: self.execute_thread(self.last_input)
    ...
    def set_expression(self, expr): 
        if expr in self.images: self.avatar_display.configure(image=self.images[expr])
    ...
    def bg_execute(self, txt):
        ...
        try:
            ...
            for cmd in commands:
                ...
                msg, success = self.automator.execute_single_step(cmd, self.ghost, ppt_win, auto_mode=self.auto_mode)
